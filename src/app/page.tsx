...
import { GraduationCap, ArrowRight, ShieldCheck, Users, Calendar, Award } from "lucide-react";
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-blue-950 to-slate-900 text-white flex flex-col justify-between">
      ...
      <header className="px-6 py-5 max-w-7xl mx-auto w-full flex items-center justify-between border-b border-white/10">
        <div className="flex items-center gap-3">
          <div className="h-11 w-11 rounded-xl bg-gradient-to-tr from-blue-600 to-indigo-500 flex items-center justify-center shadow-lg shadow-blue-500/30 border border-white/20">
            <GraduationCap className="h-6 w-6 text-white" />
          </div>
          <div>
            <h1 className="font-bold text-lg tracking-tight text-white leading-tight">
              SIM Sekolah <span className="text-blue-400 font-extrabold">PRO</span>
            </h1>
            <p className="text-xs text-slate-400">Sistem Informasi Manajemen Sekolah Terpadu</p>
          </div>
        </div>
      ...
      <main className="max-w-7xl mx-auto px-6 py-16 flex-1 flex flex-col justify-center items-center text-center">
        ...
        <div className="mt-10 flex flex-wrap items-center justify-center gap-4">
          ...
          >
            <span>Buka Portal & Form Login</span>
      ...
      <footer className="px-6 py-6 border-t border-white/10 text-center text-xs text-slate-400">
        &copy; {new Date().getFullYear()} SIM Sekolah PRO - Sistem Informasi Manajemen Sekolah Terpadu. All rights reserved.
      </footer>
    </div>
  );
}
